  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-black text-white' : 'bg-white text-black'}`}>
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-4xl font-bold mb-4">Bienvenido a FSAirlines Rewards</h1>
        <p className="text-xl mb-8">Gana medallas, obtén licencias y canjea tus puntos por productos exclusivos.</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
          <div className={`p-6 rounded-lg shadow-md ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
          ...
          <div className={`p-6 rounded-lg shadow-md ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
            ...
            <p>Obtén licencias especiales para mejorar tu experiencia de vuelo.</p>
          </div>
          <div className={`p-6 rounded-lg shadow-md ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
        ...
        <div className={`p-6 rounded-lg shadow-md ${darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
          <h2 className="text-2xl font-semibold mb-4">Últimos Vuelos Reportados</h2>
          {recentFlights.length > 0 ? (
            <ul>
              {recentFlights.map((flight) => (
                <li key={flight._id} className="mb-2">
                  <span className="font-semibold">{new Date(flight.date).toLocaleDateString()}</span> - 
